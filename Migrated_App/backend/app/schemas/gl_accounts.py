...
class GLAccountResponse(GLAccountBase):
    ...
    ledger_last: Decimal = Field(..., description="Previous period balance")
...
class GLPostingBase(BaseModel):
    ...
    post_amount: Decimal = Field(..., gt=0, description="Posting amount")
    ...
    vat_amount: Decimal = Field(Decimal('0.00'), ge=0, description="VAT amount")
...
class GLPostingResponse(GLPostingBase):
    ...
    vat_pc: int
    post_vat_side: str
    total_amount: Decimal = Field(..., description="Amount including VAT")
